...
def get_intensity_from3D(Potion):
    ...
    for i in range(0,Potion.shape[0]):
        ...
        curr_Intensity = np.zeros([curr_potion.shape[1], curr_potion.shape[2],curr_potion.shape[3]])
        for i_channel in range(np.shape(curr_potion)[0]):
...
def get_descriptor(heatmaps,C):
    ...
    potion=np.reshape(potion,(potion.shape[0]*potion.shape[1],potion.shape[2],potion.shape[3],potion.shape[4]))
...
def get_heatvolume(amplitude,center_x,center_y,center_z,resolution):
    ...
    sigma_square = 2.25
    heat_volume=np.exp(-(x**2 + y**2 + z**2)/(2.0*sigma_square))/(np.sqrt(2.0*np.pi*sigma_square))*amplitude
