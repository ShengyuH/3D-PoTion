...
local count=1
for eachfolder in lfs.dir(path_input) do 
	if(eachfolder~='.DS_Store'  and eachfolder~='.' and eachfolder~='..') then
		crndir=path_input .. '/'.. eachfolder
		local outpath=path_output .. '/' .. eachfolder .. '.h5' 
		local predFile = hdf5.open(outpath, 'w')
		print(outpath)
		count=count+1
		print(count)
		for eachfile in lfs.dir(crndir) do 
			if(eachfile~='.DS_Store' and eachfile~='.' and eachfile~='..') then
				local img_path=crndir .. '/' .. eachfile
